...
void handle_fully_connected_layer_with_backsubstitute(elina_manager_t* man, elina_abstract0_t* element, double **weights, double * cst, size_t num_out_neurons, size_t num_in_neurons, size_t * predecessors, size_t num_predecessors, bool alloc, fnn_op OP, bool has_two_slbs, bool random_prune){
    ...
    for(i=0; i < num_out_neurons; i++){
		...
		//For affine neurons, there is no two lower bounds
		out_neurons[i]->twolb_flag = false;
		out_neurons[i]->aux_lexpr = NULL;
    }
    
    update_state_using_previous_layers_parallel(man,fp,numlayers,has_two_slbs, random_prune);
    
    return;
}

void handle_sub_layer(elina_manager_t* man, elina_abstract0_t * abs,  double *cst, bool is_minuend, size_t size, size_t *predecessors, size_t num_predecessors, bool has_two_slbs, bool random_prune){
	if(is_minuend==true){
		handle_fully_connected_layer_with_backsubstitute(man, abs, NULL, cst, size, size, predecessors, num_predecessors, true, SUB1, has_two_slbs,random_prune);
	}
	else{
        	handle_fully_connected_layer_with_backsubstitute(man, abs, NULL, cst, size, size, predecessors, num_predecessors, true, SUB2, has_two_slbs, random_prune);
	}
}

void handle_mul_layer(elina_manager_t* man, elina_abstract0_t * abs, double *bias,  size_t size, size_t *predecessors, size_t num_predecessors, bool has_two_slbs, bool random_prune){
        handle_fully_connected_layer_with_backsubstitute(man, abs, NULL, bias, size, size, predecessors, num_predecessors, true, MUL, has_two_slbs, random_prune);
}

void handle_fully_connected_layer_no_alloc(elina_manager_t* man, elina_abstract0_t * abs, double **weights, double *bias,   size_t size, size_t num_pixels, size_t *predecessors, size_t num_predecessors, bool has_two_slbs, bool random_prune){
    handle_fully_connected_layer_with_backsubstitute(man, abs, weights, bias, size, num_pixels, predecessors, num_predecessors, false, MATMULT, has_two_slbs, random_prune);
}


void handle_fully_connected_layer(elina_manager_t* man, elina_abstract0_t * abs, double **weights, double *bias,   size_t size, size_t num_pixels, size_t *predecessors, size_t num_predecessors, bool has_two_slbs, bool random_prune){
     handle_fully_connected_layer_with_backsubstitute(man, abs, weights, bias, size, num_pixels, predecessors, num_predecessors, true, MATMULT, has_two_slbs, random_prune);
}
...
void *get_upper_bound_for_linexpr0_parallel(void *args){
	...
	for(i=idx_start; i < idx_end; i++){
		...
		ub = fmin(ub,get_ub_using_previous_layers(man,fp,uexpr,layerno,false,false));
...
void handle_concatenation_layer(elina_manager_t* man, elina_abstract0_t* element, size_t * predecessors, size_t num_predecessors, size_t *C){
	...
	num_channels = num_channels + C[i];
		...
		out_neurons[k]->twolb_flag = false;
		out_neurons[k]->aux_lexpr = NULL;
...
bool is_greater(elina_manager_t* man, elina_abstract0_t* element, elina_dim_t y, elina_dim_t x, bool has_two_slbs, bool random_prune){
	fppoly_t *fp = fppoly_of_abstract0(element);
	fppoly_internal_t * pr = fppoly_init_from_manager(man,ELINA_FUNID_ASSIGN_LINEXPR_ARRAY);
	//if(1){
		...
		double lb = get_lb_using_previous_layers(man, fp, sub, fp->numlayers, has_two_slbs, random_prune);
		 ...
		 for(out_z=0; out_z < output_size[2]; out_z++) {
		   ...
		   //actual_coeff is the number of parameters
	       	   out_neurons[mat_x]->lexpr = create_sparse_expr(coeff,cst,dim,actual_coeff);
       	  	   out_neurons[mat_x]->twolb_flag = false;
		   out_neurons[mat_x]->aux_lexpr = NULL;
...
void handle_batchnormalization_layer(elina_manager_t* man, elina_abstract0_t* element, double *weight_para, double * bias_para, size_t *output_size, size_t *input_size, size_t parameter_length, size_t *predecessors, size_t num_predecessors, bool has_two_slbs, bool random_prune){
	assert(num_predecessors==1);
	fppoly_t *fp = fppoly_of_abstract0(element);
	size_t numlayers = fp->numlayers;
	size_t i, j;
	...
	for(out_x=0; out_x < output_size[0]; out_x++) {
	    for(out_y = 0; out_y < output_size[1]; out_y++) {
		 for(out_z=0; out_z < output_size[2]; out_z++) {
		 	 //The one-dimensional index of the output pixel [out_x,out_y, out_z], which is also the same for input dimension
		     size_t mat_x = out_x*output_size[1]*output_size[2] + out_y*output_size[2] + out_z;
		     size_t actual_coeff = 0;
		     ...
		     // The dimension of the input is the same as input
		     dim[0] = mat_x;
		     actual_coeff++;
...
void handle_residual_layer(elina_manager_t *man, elina_abstract0_t *element, size_t num_neurons, size_t *predecessors, size_t num_predecessors, bool has_two_slbs, bool random_prune){
	...
	for(i=0; i < num_neurons; i++){
		...
		neurons[i]->uexpr = neurons[i]->lexpr;
   	        neurons[i]->twolb_flag = false;
	        neurons[i]->aux_lexpr = NULL;
	}
	//printf("FINISH\n");
	//fflush(stdout);
	update_state_using_previous_layers_parallel(man,fp,numlayers, has_two_slbs, random_prune);
...
void free_neuron(neuron_t *neuron){
	...
	if(neuron->twolb_flag){
		free_expr(neuron->aux_lexpr);
	}
	free(neuron);
}
