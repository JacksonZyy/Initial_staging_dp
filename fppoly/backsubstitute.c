...
void * update_state_using_previous_layers(void *args){
	...
	fppoly_t *fp = data->fp;
	bool has_two_slbs = data->has_two_slbs;
	bool random_prune = data->random_prune;
	...
	for(i=idx_start; i < idx_end; i++){
		...
		out_neurons[i]->lb = get_lb_using_previous_layers(man, fp, lexpr, layerno, has_two_slbs, random_prune);
		out_neurons[i]->ub = get_ub_using_previous_layers(man, fp, uexpr, layerno, has_two_slbs, random_prune);
		free_expr(lexpr);
		free_expr(uexpr);
...
void update_state_using_previous_layers_parallel(elina_manager_t *man, fppoly_t *fp, size_t layerno, bool has_two_slbs, bool random_prune){
	...
	if(num_out_neurons < NUM_THREADS){
		for (i = 0; i < num_out_neurons; i++){
			...
			args[i].res = NULL;
	    	args[i].has_two_slbs = has_two_slbs;
	    	args[i].random_prune = random_prune;
