...
if dataset=='acasxu':
    ...
    for i in range(1):
        ...
        '''
        all_rows = []
        with open("/Verifier_nn/ERAN/aux_funcs/one_dimen_cat_noimg.csv", "r") as f:
            reader = csv.reader(f, delimiter=",")
            for i, line in enumerate(reader):
                all_rows.append(line)
        downsample_img = np.asarray(all_rows[0], dtype=np.float64)
        specLB = np.copy(downsample_img)
        specUB = np.copy(downsample_img)
        specLB = specLB - epsilon
        specUB = specUB + epsilon
        specLB = np.reshape(specLB, (1, 5, 14, 14)).transpose(0,2,3,1)
        specUB  = np.reshape(specUB, (1, 5, 14, 14)).transpose(0,2,3,1)
        '''
        if (args.stage_layer_num or args.stage_block_num):
            # Handle the staging
            one_dim_LB = specLB.flatten()
            one_dim_UB = specUB.flatten()
            print("enter eran function")
            start = time.time()
            eran_result = eran.analyze_in_stages(one_dim_LB, one_dim_UB, init_domain(domain), config.timeout_lp,
                                                 config.timeout_milp, config.use_default_heuristic,
                                                 args.stage_layer_num, args.stage_block_num, args.two_lbs,
                                                 args.random_prune)
            end = time.time()
            print("leave eran function")
            nub = np.array(eran_result[1])
            nlb = np.array(eran_result[0])
            print(nlb[-1])
            print(nub[-1])
            print(end - start, "seconds")
            '''
            fullpath = "DPnet_stage_lbs.csv"
            with open(fullpath, 'a+', newline='') as write_obj:
                csv_writer = csv.writer(write_obj)
                csv_writer.writerow([net_name_list[-1],'eps:'+str(epsilon),'#layer:'+str(args.stage_layer_num), str(end - start)+' seconds','Lower'] + list(nlb[-1]))
            fullpath = "DPnet_stage_ubs.csv"
            with open(fullpath, 'a+', newline='') as write_obj:
                csv_writer = csv.writer(write_obj)
                csv_writer.writerow([net_name_list[-1],'eps:'+str(epsilon),'#layer:'+ str(args.stage_layer_num), str(end - start)+' seconds','Upper'] + list(nub[-1]))
            '''
        else:
            one_dim_LB = specLB.flatten()
            one_dim_UB = specUB.flatten()
            print("enter eran function")
            start = time.time()
            eran_result = eran.analyze_box(one_dim_LB, one_dim_UB, init_domain(domain), config.timeout_lp,
                                           config.timeout_milp, config.use_default_heuristic, args.two_lbs,
                                           args.random_prune)
            end = time.time()
            print("leave eran function")
            ...
            print(nub[-1])
            print(end - start, "seconds")
            fullpath = "determ_stage_dpnet_lbs.csv"
            with open(fullpath, 'a+', newline='') as write_obj:
                csv_writer = csv.writer(write_obj)
                csv_writer.writerow([net_name_list[-1],'eps:'+str(epsilon),'#layer:'+ 'None', str(end - start)+' seconds'] + list(nlb[-1]))
            fullpath = "determ_stage_dpnet_ubs.csv"
            with open(fullpath, 'a+', newline='') as write_obj:
                csv_writer = csv.writer(write_obj)
                csv_writer.writerow([net_name_list[-1],'eps:'+str(epsilon),'#layer:'+ 'None', str(end - start)+' seconds'] + list(nub[-1]))
