...
class Analyzer:
    def __init__(self, ir_list, nn, domain, timeout_lp, timeout_milp, output_constraints, use_default_heuristic, label, prop, testing = False, two_lbs = False, random_prune = False):
        """
        Arguments
        ---------
        ir_list: list
    ...
    def get_abstract0(self):
        ...
        testing_nlb = []
        testing_nub = []
        #print("len", len(self.ir_list))
        for i in range(1, len(self.ir_list)):
            #print(i)
            element_test_bounds = self.ir_list[i].transformer(self.nn, self.man, element, nlb, nub, self.relu_groups, 'refine' in self.domain, self.timeout_lp, self.timeout_milp, self.use_default_heuristic, self.testing, self.two_lbs, self.random_prune)
            ...
            for i in candidate_labels:
                ...
                for j in adv_labels:
                    if self.domain == 'deepzono' or self.domain == 'refinezono':
                        ...
                        if label!=j and not self.is_greater(self.man, element, label, j, self.use_default_heuristic, self.two_lbs, self.random_prune):
                            #print("branch a")
                            if(self.domain=='refinepoly'):
