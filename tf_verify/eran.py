...
class ERAN:
    ...
    def analyze_in_stages(self, specLB, specUB,  domain, timeout_lp, timeout_milp, use_default_heuristic, stage_layer_num=None, stage_block_num=None, has_two_lbs=False,
                    random_prune=False, output_constraints=None, lexpr_weights=None, lexpr_cst=None, lexpr_dim=None,
        ...
        for i in range(len(operations_list)):
            ...
            analyzer = Analyzer(execute_list, nn, domain, timeout_lp, timeout_milp, output_constraints,
                                use_default_heuristic, label, prop, testing, has_two_lbs, random_prune)
    ...
    def analyze_box(self, specLB, specUB, domain, timeout_lp, timeout_milp, use_default_heuristic, has_two_lbs = False, random_prune = False, output_constraints=None, lexpr_weights= None, lexpr_cst=None, lexpr_dim=None, uexpr_weights=None, uexpr_cst=None, uexpr_dim=None, expr_size=0, testing = False,label=-1, prop = -1):
        """
        This function runs the analysis with the provided model and session from the constructor, the box specified by specLB and specUB is used as input. Currently we have three domains, 'deepzono',      		'refinezono' and 'deeppoly'.
        ...
        if domain == 'deepzono' or domain == 'refinezono':
            ...
            analyzer = Analyzer(execute_list, nn, domain, timeout_lp, timeout_milp, output_constraints, use_default_heuristic,label, prop, testing, has_two_lbs, random_prune)
            ...
            #print("Execution list",execute_list)
            analyzer = Analyzer(execute_list, nn, domain, timeout_lp, timeout_milp, output_constraints, use_default_heuristic, label, prop, testing, has_two_lbs, random_prune)
            #print("leave Analyzer initialization")
        #print("Enter analyze() function")
        dominant_class, nlb, nub, failed_labels, x = analyzer.analyze()
        #print("Leave analyze() function")
        if testing:
