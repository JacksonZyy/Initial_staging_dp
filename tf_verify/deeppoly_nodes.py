...
class DeeppolyFCNode(DeeppolyNode):
    def transformer(self, nn, man, element, nlb, nub, relu_groups, refine, timeout_lp, timeout_milp, use_default_heuristic, testing, has_two_lbs, random_prune):
        """
        ...
        """
        handle_fully_connected_layer(man, element, *self.get_arguments(), has_two_lbs, random_prune)
...
class DeeppolyReluNode(DeeppolyNonlinearity):
    def transformer(self, nn, man, element, nlb, nub, relu_groups, refine, timeout_lp, timeout_milp, use_default_heuristic, testing, has_two_lbs, random_prune):
        ...
        if refine:
            ...
            #print("Enable two lbs and enter relu_layer_twolb")
            handle_relu_layer_twolb(*self.get_arguments(man, element), use_default_heuristic)
            #print("Enable two lbs and leave relu_layer_twolb")
        else:
            #print("Disable two lbs and enter relu_layer")
            handle_relu_layer(*self.get_arguments(man, element), use_default_heuristic)
            #print("Disable two lbs and leave relu_layer")
...
class DeeppolyConv2dNode:
    ...
    def transformer(self, nn, man, element, nlb, nub, relu_groups, refine, timeout_lp, timeout_milp, use_default_heuristic, testing, has_two_lbs, random_prune):
        ...
        output : ElinaAbstract0Ptr
            abstract element after the transformer 
        """
        #print("Deeppoly node call handle_convolutional_layer with flag two_lbs equals to ", has_two_lbs)
        handle_convolutional_layer(man, element, *self.get_arguments(), has_two_lbs, random_prune)
...
class DeeppolyBNNode:
    def __init__(self, scale_para, bias_para, mean_para, var_para, epsilon, image_shape, input_names, output_name, output_shape):
        """
        collects the information needed for the bn_handle_intermediate_relu_layer transformer and brings it into the required shape
    ...
    def transformer(self, nn, man, element, nlb, nub, relu_groups, refine, timeout_lp, timeout_milp,
                    use_default_heuristic, testing, has_two_lbs, random_prune):
        """
        ...
        output : ElinaAbstract0Ptr
            abstract element after the transformer
        """
        handle_batchnormalization_layer(man, element, *self.get_arguments(), has_two_lbs, random_prune)
        calc_bounds(man, element, nn, nlb, nub, relu_groups, is_refine_layer=True)
        #nn.conv_counter += 1
        nn.ffn_counter += 1
        if testing:
...
class DeeppolyResidualNode:
    ...
    def transformer(self, nn, man, element, nlb, nub, relu_groups, refine, timeout_lp, timeout_milp, use_default_heuristic, testing, has_two_lbs, random_prune):
        handle_residual_layer(man,element,self.output_length,self.predecessors, len(self.predecessors), has_two_lbs, random_prune)
...
class DeeppolySubNode:
    ...
    def transformer(self, nn, man, element, nlb, nub, relu_groups, refine, timeout_lp, timeout_milp, use_default_heuristic, testing, has_two_lbs, random_prune):
        layerno = nn.calc_layerno()
        num_neurons = get_num_neurons_in_layer(man, element, layerno)
        handle_sub_layer(man, element, self.bias, self.is_minuend, num_neurons, self.predecessors, len(self.predecessors), has_two_lbs, random_prune)
        calc_bounds(man, element, nn, nlb, nub, relu_groups, is_refine_layer=True)
        nn.ffn_counter+=1
        if testing:
            return element, nlb[-1], nub[-1]
...
class DeeppolyMulNode:
    ...
    def transformer(self, nn, man, element, nlb, nub, relu_groups, refine, timeout_lp, timeout_milp, use_default_heuristic, testing, has_two_lbs, random_prune):
        handle_mul_layer(man, element, self.bias, len(self.bias.reshape(-1)), self.predecessors, len(self.predecessors), has_two_lbs, random_prune)
        calc_bounds(man, element, nn, nlb, nub, relu_groups, is_refine_layer=True)
        nn.ffn_counter+=1
        if testing:
            return element, nlb[-1], nub[-1]
