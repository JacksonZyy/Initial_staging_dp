...
if net_file in ['FCN1.onnx']:
    specLB = np.asarray([-2,-2,-2,-2,-2], dtype=np.float64)
    specUB = np.asarray([2,2,2,2,2], dtype=np.float64)
elif net_file in ['FCN2.onnx']:
    specLB = np.asarray([-2,-2,-2,-2,-2,-2], dtype=np.float64)
    specUB = np.asarray([2,2,2,2,2,2], dtype=np.float64)
elif net_file in ['FCN3.onnx']:
    specLB = np.asarray([-2,-2,-2,-2,-2,-2,-2], dtype=np.float64)
    specUB = np.asarray([2,2,2,2,2,2,2], dtype=np.float64)
elif net_file in ['FCN4.onnx', 'CNN2.onnx', 'CNN3.onnx']:
    specLB = np.asarray([-2,-2], dtype=np.float64)
    specUB = np.asarray([2,2], dtype=np.float64)
elif net_file in ['FCN5.onnx', 'CNN1.onnx', 'CNN4.onnx','merge_FCN1.onnx']:
    specLB = np.asarray([-2,-2,-2,-2], dtype=np.float64)
    specUB = np.asarray([2,2,2,2], dtype=np.float64)
...
if(args.stage_layer_num or args.stage_block_num):
    ...
    start = time.time()
    eran_result = eran.analyze_in_stages(one_dim_LB, one_dim_UB, init_domain(domain), config.timeout_lp, config.timeout_milp, config.use_default_heuristic, args.stage_layer_num, args.stage_block_num, args.two_lbs, args.random_prune)
    end = time.time()
