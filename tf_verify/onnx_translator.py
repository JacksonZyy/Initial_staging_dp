...
def prepare_model(model):
    ...
    for node in model.graph.node:
        ...
        if node.op_type == "Flatten":
            ...
            if len(shape_map[node.input[1]]) == 2:
                # The first dimension of para matrix should be the same as the last
                assert shape_map[node.input[0]][0] == shape_map[node.input[1]][0]
                shape_map[node.output[0]] = [shape_map[node.input[1]][1]]
                continue
            transA = 0
            transB = 0
    ...
    def staging_trans(self, stage_layer_num, stage_block_num):
        ...
        for node in self.nodes:
            if node.op_type == "Constant":
                ...
                continue
            if (node.op_type not in ["Relu", "Sigmoid", "Tanh"]):
            ...
            if node.op_type == "MatMul":
                ...
                if node.output[0] in self.BN_map:
                    # This Conv is the preceding layer of BN layer
                    filters, bias = self.conca_conv_BN(node, filters, bias)
                # print(bias)
                deeppoly_res = (filters, bias, image_shape, strides, pad_top, pad_left) + in_out_info
                # print ("deeppoly_res ",deeppoly_res)
                deepzono_res = deeppoly_res
                operation_resources.append({'deepzono': deepzono_res, 'deeppoly': deeppoly_res})
                ...
                # padding is expected to be string in tf. dilations, auto_pad, ceil_mode, storage_order are unused at the moment
                ...
                if(stage_layer_num):
                    ...
                    if(layer_counter == stage_layer_num):
                        ...
                        operation_types = ["Placeholder"]
                        in_out_placeholder = ([], node.output[0], self.shape_map[node.output[0]])
                        operation_resources = [{'deepzono': in_out_placeholder, 'deeppoly': in_out_placeholder}]
                        layer_counter = 0
            elif node.op_type == "Gather":
                ...
                if node.output[0] in self.input_node_map and self.input_node_map[node.output[0]].op_type in [
                    "MatMul",
                    "Gemm"]:
                    self.ignore_node(node, operation_types, reshape_map)

                elif node.output[0] in self.input_node_map and self.input_node_map[node.output[0]].op_type in [
                    "Relu",
                    "Sigmoid",
                    "Tanh"] and \
...
if __name__ == '__main__':
    onnx_file = "/Users/zhongyuyi/Desktop/eran-master/Two_lbs/Multiple network testing/FCN4.onnx"
    ...
    '''
    translator = ONNXTranslator(onnx_model)
    operations, resources = translator.staging_trans(3,None)
    '''
    for node in onnx_model.graph.node:
        print(node.op_type)
    ...
    input = np.array([1., 1.], dtype=np.float32)
